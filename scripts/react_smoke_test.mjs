/* 余响 Encore v2.0.0 · React 重构冒烟测试
 * 启动无头 Chrome（CDP），逐页加载并检查：
 * 1) 控制台异常 / 错误日志
 * 2) 关键 DOM 标记是否存在
 * 3) 核心交互是否可用
 * 4) 输出页面截图
 * 用法：node scripts/react_smoke_test.mjs
 */
...
const FORCE_THEME = process.env.SMOKE_THEME === 'light'
  ? `try { localStorage.setItem('encore-theme', 'light'); } catch (e) {} if (document.documentElement) document.documentElement.classList.remove('dark');`
  : `try { localStorage.setItem('encore-theme', 'dark'); } catch (e) {} if (document.documentElement) document.documentElement.classList.add('dark');`;
const STUB_FETCH_SOURCE = `${FORCE_THEME}
(() => {
...
async function main() {
  ...
  try {
    for (const page of PAGES) {
      ...
      const appHtml = await evalIn(cdp, sessionId, `document.getElementById('root') ? document.getElementById('root').innerHTML.length : -1`);
        ...
        if (page.file === 'index.html') {
          interaction = await evalIn(cdp, sessionId, `(function(){
            ...
            return new Promise(function(resolve){
              let attempts = 0;
              function tick(){
                if (fortuneBtn && attempts < 5) {
                  fortuneBtn.click();
                  attempts += 1;
                }
                if (attempts >= 5 || document.querySelector('.fortune-artist').textContent !== fortuneBefore) {
                  resolve({
                    dark,
                    fortuneBtnExists: !!fortuneBtn,
                    fortuneBefore,
                    fortuneAfter: document.querySelector('.fortune-artist').textContent,
                    fortuneNonceAfter: sessionStorage.getItem('encore-fortune-nonce'),
                    fortuneChanged: fortuneBefore !== document.querySelector('.fortune-artist').textContent
                  });
                  return;
                }
                setTimeout(tick, 250);
          ...
          interaction = await evalIn(cdp, sessionId, `(function(){
            ...
            return new Promise(function(resolve){
              setTimeout(function(){
                ...
                const modalOpen = document.querySelector('.modal-backdrop.open') !== null;
                if (date) {
                  Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, 'value').set.call(date, '2026-08-20');
                  date.dispatchEvent(new Event('input', { bubbles: true }));
                  date.dispatchEvent(new Event('change', { bubbles: true }));
                }
                if (name) {
                  Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, 'value').set.call(name, '测试见面');
                ...
                setTimeout(function(){
                  ...
                  const meetDatesStorage = localStorage.getItem('encore-meet-dates');
                  ...
                  setTimeout(function(){
                    ...
                    setTimeout(function(){
                      resolve({
                        ...
                        filled,
                        meetDatesStorage,
                        cleared: !document.querySelector('.meet-num')
                      });
                    }, 300);
                  }, 300);
                }, 300);
          ...
          interaction = await evalIn(cdp, sessionId, `(function(){
            ...
            return new Promise(function(resolve){
              setTimeout(function(){
                ...
                if (name) {
                  Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, 'value').set.call(name, '测试消费');
                  name.dispatchEvent(new Event('input', { bubbles: true }));
                }
                if (amount) {
                  Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, 'value').set.call(amount, '520');
                  amount.dispatchEvent(new Event('input', { bubbles: true }));
                }
                if (date) {
                  Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, 'value').set.call(date, '2026-08-06');
                  date.dispatchEvent(new Event('input', { bubbles: true }));
                  date.dispatchEvent(new Event('change', { bubbles: true }));
                }
